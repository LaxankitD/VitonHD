...
def test(opt, seg, gmm, alias):
    ...
    with torch.no_grad():
        for i, inputs in enumerate(test_loader.data_loader):
            ...
            # Move tensors to CPU using the selected device
            ...
            seg_input = torch.cat((cm_down, c_masked_down, parse_agnostic_down, pose_down, gen_noise(cm_down.size()).to(device)),
                                  dim=1)
            ...
            # Move tensors to CPU
            img_agnostic_cpu = img_agnostic.to(device)
            pose_cpu = pose.to(device)
            warped_c_cpu = warped_c.to(device)
            parse_cpu = parse.to(device)
            parse_div_cpu = parse_div.to(device)
            misalign_mask_cpu = misalign_mask.to(device)

            alias = alias.to(device)

            # Concatenate tensors on CPU
            concatenated_input = torch.cat((img_agnostic_cpu, pose_cpu, warped_c_cpu), dim=1)
            # Pass the concatenated input and other tensors to the alias model
            output = alias(concatenated_input, parse_cpu, parse_div_cpu, misalign_mask_cpu)
...
def main():
    ...
    # Check if CUDA (GPU) is available, and use it if available, otherwise use CPU
    if torch.cuda.is_available():
        seg.cuda().eval()
        gmm.cuda().eval()
        alias.cuda().eval()
